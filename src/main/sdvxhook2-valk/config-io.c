...
#define SDVXHOOK2_VALK_CONFIG_IO_DISABLE_CARD_READER_EMU_KEY \
    "io.disable_card_reader_emu"
#define SDVXHOOK2_VALK_CONFIG_IO_DISABLE_BIO2_EMU_KEY "io.disable_bio2_emu"
#define SDVXHOOK2_VALK_CONFIG_IO_DISABLE_POLL_LIMITER_KEY "io.disable_poll_limiter"
#define SDVXHOOK2_VALK_CONFIG_IO_FORCE_HEADPHONES_KEY "io.force_headphones"
...
#define SDVXHOOK2_VALK_CONFIG_IO_DEFAULT_DISABLE_POLL_LIMITER_VALUE false
#define SDVXHOOK2_VALK_CONFIG_IO_DEFAULT_FORCE_HEADPHONES_VALUE false
...
void sdvxhook2_valk_config_io_init(struct cconfig *config)
{
    cconfig_util_set_bool(
        config,
        SDVXHOOK2_VALK_CONFIG_IO_DISABLE_CARD_READER_EMU_KEY,
        SDVXHOOK2_VALK_CONFIG_IO_DEFAULT_DISABLE_CARD_READER_EMU_VALUE,
        "Disable card reader emulation and enable usage of real card reader "
        "hardware on COM2");

    cconfig_util_set_bool(
        config,
        SDVXHOOK2_VALK_CONFIG_IO_DISABLE_BIO2_EMU_KEY,
        SDVXHOOK2_VALK_CONFIG_IO_DEFAULT_DISABLE_BIO2_EMU_VALUE,
        "Disable BIO2 emulation and enable usage of real BIO2 hardware");

    cconfig_util_set_bool(
        config,
        SDVXHOOK2_VALK_CONFIG_IO_DISABLE_POLL_LIMITER_KEY,
        SDVXHOOK2_VALK_CONFIG_IO_DEFAULT_DISABLE_POLL_LIMITER_VALUE,
        "Disables the poll limiter, warning very high CPU usage may arise");

    cconfig_util_set_bool(
        config,
        SDVXHOOK2_VALK_CONFIG_IO_FORCE_HEADPHONES_KEY,
        SDVXHOOK2_VALK_CONFIG_IO_DEFAULT_FORCE_HEADPHONES_VALUE,
        "Forces game to think headphones are attached");

    cconfig_util_set_bool(
        config,
        SDVXHOOK2_VALK_CONFIG_IO_DISABLE_FILE_HOOKS_KEY,
        SDVXHOOK2_VALK_CONFIG_IO_DEFAULT_DISABLE_FILE_HOOKS_VALUE,
        "Disables the built in file hooks, requiring manual file creation "
        "(/dev/raw/j.dest)");

    cconfig_util_set_bool(
        config,
        SDVXHOOK2_VALK_CONFIG_IO_DISABLE_POWER_HOOKS_KEY,
        SDVXHOOK2_VALK_CONFIG_IO_DEFAULT_DISABLE_POWER_HOOKS_VALUE,
        "Disables the built in power control hooks, allowing game to modify "
        "system power settings");

    cconfig_util_set_bool(
        config,
        SDVXHOOK2_VALK_CONFIG_IO_DISABLE_NVAPI_HOOKS_KEY,
