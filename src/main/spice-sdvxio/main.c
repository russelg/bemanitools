...
void handle_lights(struct spice_connection *connection, uint16_t *gpio_lights)
{
    ...
    if (spice_lights_read(connection, &light_states, &light_count)) {
        for (size_t i = 0; i < light_count; i++) {
            ...
            /*if (!strcmp(light->name, "BT-A")) {
                gpio_lights |= assign_gpio(light->value > 0,
            SDVX_IO_OUT_GPIO_A); } else if (!strcmp(light->name, "BT-B")) {
                gpio_lights |= assign_gpio(light->value > 0,
            SDVX_IO_OUT_GPIO_B); } else if (!strcmp(light->name, "BT-C")) {
                gpio_lights |= assign_gpio(light->value > 0,
            SDVX_IO_OUT_GPIO_C); } else if (!strcmp(light->name, "BT-D")) {
                gpio_lights |= assign_gpio(light->value > 0,
            SDVX_IO_OUT_GPIO_D); } else if (!strcmp(light->name, "FX-L")) {
                gpio_lights |= assign_gpio(light->value > 0,
            SDVX_IO_OUT_GPIO_FX_L); } else if (!strcmp(light->name, "FX-R")) {
                gpio_lights |= assign_gpio(light->value > 0,
            SDVX_IO_OUT_GPIO_FX_R); } else if (!strcmp(light->name, "Start")) {
                gpio_lights |= assign_gpio(light->value > 0,
            SDVX_IO_OUT_GPIO_START); } else if (!strcmp(light->name, "Generator
            B")) { gpio_lights |= assign_gpio(        light->value > 0,
            SDVX_IO_OUT_GPIO_GENERATOR_B); } else if (!strcmp(light->name, "Left
            Wing Avg R")) {
                // "Wing Left Up R"
                sdvx_io_set_pwm_light(0, rescale_light_value(light->value));
            } else if (!strcmp(light->name, "Left Wing Avg G")) {
                // "Wing Left Up G"
                sdvx_io_set_pwm_light(1, rescale_light_value(light->value));
            } else if (!strcmp(light->name, "Left Wing Avg B")) {
                // "Wing Left Up B"
                sdvx_io_set_pwm_light(2, rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Right Wing Avg R")) {
                //     // "Wing Right Up R"
                //     sdvx_io_set_pwm_light(3,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Right Wing Avg G")) {
                //     // "Wing Right Up G"
                //     sdvx_io_set_pwm_light(4,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Right Wing Avg B")) {
                //     // "Wing Right Up B"
                //     sdvx_io_set_pwm_light(5,
            rescale_light_value(light->value)); } else if (!strcmp(light->name,
            "V Unit Avg R")) { uint8_t value =
            rescale_light_value(light->value);
                // "Wing Left Low R"
                sdvx_io_set_pwm_light(6, value);
                // "Wing Right Low R"
                // sdvx_io_set_pwm_light(9, value);
            } else if (!strcmp(light->name, "V Unit Avg G")) {
                uint8_t value = rescale_light_value(light->value);
                // "Wing Left Low G"
                sdvx_io_set_pwm_light(7, value);
                // "Wing Right Low G"
                // sdvx_io_set_pwm_light(10, value);
            } else if (!strcmp(light->name, "V Unit Avg B")) {
                uint8_t value = rescale_light_value(light->value);
                // "Wing Left Low B"
                sdvx_io_set_pwm_light(8, value);
                // "Wing Right Low B"
                // sdvx_io_set_pwm_light(11, value);
            } else if (!strcmp(light->name, "Woofer Avg R")) {
                // "Woofer R"
                sdvx_io_set_pwm_light(12, rescale_light_value(light->value));
            } else if (!strcmp(light->name, "Woofer Avg G")) {
                // "Woofer G"
                sdvx_io_set_pwm_light(13, rescale_light_value(light->value));
            } else if (!strcmp(light->name, "Woofer Avg B")) {
                // "Woofer B"
                sdvx_io_set_pwm_light(14, rescale_light_value(light->value));
            } else if (!strcmp(light->name, "Control Panel Avg R")) {
                // "Controller R"
                sdvx_io_set_pwm_light(15, rescale_light_value(light->value));
            } else if (!strcmp(light->name, "Control Panel Avg G")) {
                // "Controller G"
                sdvx_io_set_pwm_light(16, rescale_light_value(light->value));
            } else if (!strcmp(light->name, "Control Panel Avg B")) {
                // "Controller B"
                sdvx_io_set_pwm_light(17, rescale_light_value(light->value));
            } else if (!strcmp(light->name, "Generator R")) {
                // "Generator R"
                sdvx_io_set_pwm_light(18, rescale_light_value(light->value));
            } else if (!strcmp(light->name, "Generator G")) {
                // "Generator G"
                sdvx_io_set_pwm_light(19, rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Wing Left Up R")) {
                //     sdvx_io_set_pwm_light(0,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Wing Left Up G")) {
                //     sdvx_io_set_pwm_light(1,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Wing Left Up B")) {
                //     sdvx_io_set_pwm_light(2,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Wing Right Up R")) {
                //     sdvx_io_set_pwm_light(3,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Wing Right Up G")) {
                //     sdvx_io_set_pwm_light(4,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Wing Right Up B")) {
                //     sdvx_io_set_pwm_light(5,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Wing Left Low R")) {
                //     sdvx_io_set_pwm_light(6,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Wing Left Low G")) {
                //     sdvx_io_set_pwm_light(7,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Wing Left Low B")) {
                //     sdvx_io_set_pwm_light(8,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Wing Right Low R")) {
                //     sdvx_io_set_pwm_light(9,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Wing Right Low G")) {
                //     sdvx_io_set_pwm_light(10,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Wing Right Low B")) {
                //     sdvx_io_set_pwm_light(11,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Woofer R")) {
                //     sdvx_io_set_pwm_light(12,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Woofer G")) {
                //     sdvx_io_set_pwm_light(13,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Woofer B")) {
                //     sdvx_io_set_pwm_light(14,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Controller R")) {
                //     sdvx_io_set_pwm_light(15,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Controller G")) {
                //     sdvx_io_set_pwm_light(16,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Controller B")) {
                //     sdvx_io_set_pwm_light(17,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Generator R")) {
                //     sdvx_io_set_pwm_light(18,
            rescale_light_value(light->value));
                // } else if (!strcmp(light->name, "Generator G")) {
                //     sdvx_io_set_pwm_light(19,
            rescale_light_value(light->value)); } else if (!strcmp(light->name,
            "Pop")) {
                // TODO: you could grab "light->value" e [0,1] here for
                // custom lights
            } else if (!strcmp(light->name, "Title Left")) {
                // TODO: you could grab "light->value" e [0,1] here for
                // custom lights
            } else if (!strcmp(light->name, "Title Right")) {
                // TODO: you could grab "light->value" e [0,1] here for
                // custom lights
            } else if (!strcmp(light->name, "Volume Sound")) {
                volume[0] = 100 - (int) (light->value * 100);
            } else if (!strcmp(light->name, "Volume Headphone")) {
                volume[1] = 100 - (int) (light->value * 100);
            } else if (!strcmp(light->name, "Volume Woofer")) {
                volume[2] = 100 - (int) (light->value * 100);
            }*/
        }
